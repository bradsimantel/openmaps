...
Status: historical organization record. See the current
[configuration guide](../../config/README.md) for later path and naming changes.
...
qualification code under `internal/routing/qualification/testdata/`. The July
Newport refresh rehearsal and Bremen Scout experiment live beside the historical
records that explain them in `docs/log/`.

The Places/geocoding config now carries the expected normalized bundle SHA-256
alongside the pinned source releases and source checksums. This removes the two
standalone checksum sidecars. `prepare`, `import` and `refresh build` all read the
same config field. `prepare -update-config` is the explicit maintainer operation
for accepting reviewed source exports and the resulting bundle checksum.
