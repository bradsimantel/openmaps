// places-geocoding-compact builds the non-default Parquet plus compact SQLite
// proof artifact. The production server continues to use one SQLite snapshot.
package main
...
func run() error {
	...
	out := flag.String("out", "data/openmaps-compact", "new output artifact directory")
	flag.Parse()
	if flag.NArg() != 0 {
	...
	if err = compact.Build(context.Background(), *out, bundle); err != nil {
		return err
	}
	manifestOut, err := compact.Verify(*out)
	if err != nil {
		return err
	}
	var bytes int64
	...
	for _, file := range manifestOut.Files {
		...
		bytes += stat.Size()
		if file.Name == compact.EntitiesName {
			entities = file.Rows
		}
	}
	stat, err := os.Stat(filepath.Join(*out, compact.ManifestName))
	if err != nil {
		return err
	}
	bytes += stat.Size()
