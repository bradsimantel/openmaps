...
export function initializeRouting({getMap,getMapLibre,request}) {
 ...
 async function health(){try{const h=await request('/healthz');available=h.routing_available===true;durationAvailable=h.routing_duration_available===true;if(!available)invalidate('Routing unavailable in this snapshot. Load a routing candidate to calculate routes.');else if(!geometry&&!busy)status.textContent='Choose both endpoints, then calculate a driving route.';update();}catch{available=null;update();}}
 ...
 submit.addEventListener('click',async()=>{
  ...
  try{
   ...
   const duration=Number.isFinite(seconds)?`${seconds===0?'0 min':seconds<60?'Less than 1 min':`About ${Math.round(seconds/60)} min`} estimated driving time (no live traffic).`:'Travel-time estimate unavailable in this retained distance-routing snapshot.';
