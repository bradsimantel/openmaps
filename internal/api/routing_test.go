...
func TestRouteContract(t *testing.T) {
	...
	for name, body := range map[string]string{"unknown body": strings.Replace(routeBody, `"polylineEncoding"`, `"trafficModel":"BEST_GUESS","polylineEncoding"`, 1), "duration input": strings.Replace(routeBody, `"origin"`, `"departureTime":"2026-09-07T12:00:00Z","origin"`, 1), "address": strings.Replace(routeBody, `"location":{"latLng":{"latitude":0,"longitude":0}}`, `"address":"50 Bellevue Ave"`, 1), "partial coordinate": strings.Replace(routeBody, `"latitude":0,`, "", 1), "null coordinate": strings.Replace(routeBody, `"latitude":0`, `"latitude":null`, 1), "uppercase coordinate": strings.Replace(routeBody, `"latitude"`, `"Latitude"`, 1), "duplicate coordinate": strings.Replace(routeBody, `"latitude":0`, `"latitude":0,"latitude":1`, 1), "missing encoding": strings.Replace(routeBody, `,"polylineEncoding":"GEO_JSON_LINESTRING"`, "", 1), "invalid latitude": strings.Replace(routeBody, `"latitude":0`, `"latitude":91`, 1), "trailing JSON": routeBody + `{}`, "null": "null", "oversize": strings.Repeat(" ", 17000) + routeBody} {
